...
class AuthAclController extends AuthAclAppController
{
    ...
    public function dashboard2($pagination = 20, $uf = ''){
        ...
        $dataChamado = $this->dataDashboard($pagination, $uf);
        $atendimento = array();
    ...
    private function dataDashboard($limit = 10, $uf = ''){
        ...
        switch ($auth_user_group['id']) {
            ...
            case '9':
                ...
                if($uf != ''){
                    $conditions[] = array('Chamado.UF' =>  $uf);
                }

                $chamados = $this->Chamado->find('all', array(
                    'contain' => array('Equipamento', 'Atendimento'),
        ...
        $this->set('validationLgpd', $this->Session->read('validation_lgpd'));
        $this->set('dataLgpd', $this->getDataLgpd($auth_user['User']['id']));
    ...
    public function getDataLgpd($id){
        $this->loadModel('Pws.UsersLgpd');

        // consulta os registro de LGPD do usuário
        $usersLgpd = $this->UsersLgpd->find('first', array('conditions' => array(
            'UsersLgpd.user_id' => $id,
        )));

        $usersLgpd = count($usersLgpd) > 0 ? $usersLgpd['UsersLgpd'] : array();

        if(count($usersLgpd) > 0){
            $usersLgpd['open'] = false;

            // valida os dados para abertura da janela
            if(($usersLgpd['politica_privacidade'] == null || $usersLgpd['termo_uso'] == null || $usersLgpd['termo_consentimento'] == null) && $usersLgpd['negado'] == 0){
                $usersLgpd['open'] = true;
            }
        }

        // retorna os dados em json
        return count($usersLgpd) > 0 ? json_encode($usersLgpd) : json_encode(array('open' => true));
    }

    public function setDataLgpd(){
        $this->loadModel('Pws.UsersLgpd');
        ...
        foreach($this->request->data['itens'] as $key => $data){
        ...
        if(isset($response['id'])){
            // seta o id
            $this->UsersLgpd->id = $response[id];

            // remove o id do array
            unset($response[id]);
        }

        $this->request->data('UsersLgpd.user_id', $userId);

        $validate = array('politica_privacidade', 'termo_uso', 'termo_consentimento');

        foreach($response as $key => $data){
            $this->request->data("UsersLgpd.{$key}", in_array($key, $validate) ? date('Y-m-d H:i:s') : $data);
        }

        // salva os dados
        if($this->UsersLgpd->save($this->request->data)){
            return true;
        }else{
            return false;
        }
