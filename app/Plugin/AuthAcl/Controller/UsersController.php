...
class UsersController extends AuthAclAppController
{
    public function beforeFilter()
    {
        ...
        $this->Auth->allow('loginBloqueado');
        $this->Auth->allow('createToken');
        $this->Auth->allow('signInByToken');
    }
    ...
    public function add()
    {
        ...
        $auth_user_group = $this->Session->read('auth_user_group');
         ...
         // consulta os tipos de email
         $responseTpEmail = $this->User->Tpemail->find('list', array('fields' => array('Tpemail.id', 'Tpemail.tipo'), 
                'conditions' => array('OR' => array(array('Tpemail.id_parent' => null), array('Tpemail.id_parent <>' => '-1'))), 
                'order' => array('Tpemail.tipo' => 'ASC'))
        );

        $this->set('tpemail', $responseTpEmail);
    ...
    public function login($isToken = false, $dataUser = array())
    {
        ...
        if ($this->request->is('post') || ($this->request->is('get') && $isToken == true)) {

            if($this->request->is('get') && $isToken == true){
                $this->request->data['User']['user_email'] = $dataUser['user_email'];
                $this->request->data['User']['user_password'] = $dataUser['user_password'];
            ...
            if (!empty($user)) {
                if (!empty($user['User']['user_code'])) {
                    ...
                    if($user['User']['change_password'] == 1){
                        ...
                        // consulta o primeiro usuário adim revenda
                        $query = $this->User->query("SELECT u.id, u.user_email FROM users u
                                                        WHERE u.id = (SELECT g.user_id FROM users_groups g WHERE g.group_id = 6 AND g.user_id = u.id LIMIT 1)
                                                        AND u.empresa_id = {$user['User']['empresa_id']}
                                                        LIMIT 1");
                        
                        // seta o email do admin
                        $emailAdmin = count($query) > 0 ? $query[0]['u']['user_email'] : '';
    ...
    public function allowApp()
    {
        ...
        // busca as solicitações de liberação
        $this->set('objData', $user->getPermissionRequestsApp());

    }

    public function setPermissionApp()
    {   
        $this->autoRender = false;
        $this->layout = null;
        $auth_user = $this->Session->read('auth_user');
        ...
        $user->setPermissionApp($_POST['id'], $auth_user['User']['id']);
    ...
    public function loginApp()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $versionApp  = isset($this->request->query['version_app']) ? $this->request->query['version_app'] : ''; // versão app
            ...
            $query = $this->User->query("SELECT u.id, u.cliente_id,  tecnico_id, u.user_name, user_status, empresa_id, filial_id, filial, g.group_id, e.logo, e.empresa_fantasia "
            ...
            if (count($query) > 0) {
                ...
                if ($query["g"]["group_id"] ==  2 || $query["g"]["group_id"] == 3) {
                    ...
                    if ($query["u"]["user_status"] == 1) {
                        ...
                        // consulta clientes relacionados para o usuário
                        $queryClientes = $this->User->query("SELECT clientes.id, clientes.CDCLIENTE, clientes.FANTASIA FROM users_clientes INNER JOIN clientes ON clientes.id = users_clientes.cliente_id WHERE users_clientes.user_id = {$query['u']['id']}");
                        
                        $arrClientes = array();

                        foreach($queryClientes as $key => $data){
                            $arrClientes[] = array('id' => $data['clientes']['id'], 'cdCliente' => $data['clientes']['CDCLIENTE'], 'fantasia' => $data['clientes']['FANTASIA']);
                        }

                        // seta as empresas do usuário
                        $query['u']['empresas'] = $arrEmpresas;
                        $query['u']['clientes'] = $arrClientes;
                        $query['u']['info_empresa'] = $query['e'];
                        ...
                        if($version != '' && $android != ''){
                            ...
                            $this->User->query("UPDATE users SET app_version_api = '{$version}', app_version = '{$versionApp}', android_version = '{$android}' WHERE id = {$query['u']['id']}");
    ...
    public function createToken(){
        ...
        if ($this->request->is('post')) {
            ...
            $user = $this->User->find('first', array(
                'conditions' => array(
                    'user_email' => $email,
                    'user_password' => AuthComponent::password($password)
                )
            ));
            ...
            if($user){
                ...
                // TODO::implementar a validação da empresa

                // criar o token de acesso
                $token = AuthComponent::password($user['user_password'].$user['user_id'].date('d-m-Y H:i:s')) . '.' . base64_encode($password);
                // validade do token 1 minuto
                $validateToken = date('Y-m-d H:i:s', strtotime("+1 minutes", strtotime(date('Y-m-d H:i:s'))));

                // remove dados desnecessários
                unset($this->request->data['User']);

                // grava os dados na tabela de usuário
                $this->request->data['token_2'] = $token;
                $this->request->data['validate_token_2'] = $validateToken;
                ...
                if($this->User->save(($this->request->data))){
                    ...
                    $token = '';
                    $message = 'Não foi possível criar o token de acesso';
                }
        ...
        // define o json
        $jsonResult   = array("success" => $success, "message" => $message, "token" => $token);
    ...
    public function signInByToken()
    {
        ...
        $user = $this->User->find('first', array(
            'conditions' => array(
                'user_email' => $email,
                'token_2' => $token,
                'validate_token_2 >=' => date('Y-m-d H:i:s')
            )
        ));
        ...
        if($user){
            ...
            // TODO: AJUSTAR
            // remonta a senha pelo token
            $password = explode('.', $token);

            $this->deleteAuth();

            // simula o login
            $this->login(true, array('user_email' => $user['User']['user_email'], 'user_password' => isset($password[1]) ? base64_decode($password[1]) : 0));
        ...
        // define o json
        $jsonResult = array("success" => $success, "message" => $message,);
