...
class AuthAclController extends AuthAclAppController
{
    ...
    public function dashboard2(){
        ...
        $dataChamado = $this->dataDashboard(20);
        $atendimento = array();
        ...
        foreach($dataChamado['chamados'] as $key => $data){
            if($data['Chamado']['STATUS'] == 'O'){
                ...
                $dataAtendimento = $this->Chamado->query("SELECT DTATENDIMENTO, HRATENDIMENTO FROM atendimentos WHERE chamado_id = '{$data['Chamado']['id']}' AND ID_BASE = '{$this->matriz}' ORDER BY id ASC limit 1");
