...
class equipamentosMedG
{
    ...
    /** Tabela de equipamentos medidores garantia
     * @param $index
     * @param $evt
     * @param $post
     * @return string
    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            ...
            case 2:
                ...
                $condition = 'ID_EQUIP_MED_G =' . $dados['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id; // Atualizar com id do ilux
                ...
                if ($save > 0) {
                    $colunas = array('id', 'ID_EQUIP_MED_G');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['ID_EQUIP_MED_G'],
                            'SEQWEB' => $return['id']
                        )
                    );
//                    gravarLog("Tabela: $table - " . $json . " - " . "ID_EQUIP_MED_G: " . $dados['ID_EQUIP_MED_G'] . " -  Empresa: " . $this->empresa_id . "; \n", $table);
...
//
            ...
            case 3: // Excluir registro
                ...
                //$condition = 'ID_EQUIP_MED_G= ' . $post['SEQILX'] . ' AND id= ' . $post['SEQWEB'];

                $condition = 'ID_EQUIP_MED_G =' . $post['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id; // Atualizar com id do ilux
    ...
    //                gravarLog("Tabela: $table - " . "ID_EQUIP_MED_G: " . $post['SEQILX'] . "empresa = " . $this->empresa_id . "/n" . $json . "/n", $table);

                    return $json;
                } else {
                    if ($save >= 0) {
                        $json = json_encode(array('MSG' => 'Sucesso'));
      //                  gravarLog("Tabela: $table - " . "ID_EQUIP_MED_G: " . $post['SEQILX'] . " - " . "empresa = " . $this->empresa_id . " - " . $json, $table);
                        return $json;
                    }
                }
                break;
