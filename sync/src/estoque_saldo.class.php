...
class estoqueSaldo extends RestController
{
    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            case 2:
                ...
                $condition = 'ID_PRODUTO_SALDOS= ' . $dados['SEQILX'];
                ...
                if ($save > 0) {
                    $colunas = array('id', 'ID_PRODUTO_SALDOS');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['ID_PRODUTO_SALDOS'],
                            'SEQWEB' => $return['id']
                        )
                    );
            ...
            case 3: // Excluir registro
                ...
                $condition = 'ID_PRODUTO_SALDOS= ' . $post['SEQILX'];
