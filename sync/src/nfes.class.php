 ...
 * Time: 16:38
 */
class nfes extends RestController
{
    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            ...
            case 2:
                ...
                $condition = 'SEQINCNFS =' . $dados['SEQILX'];
                ...
                if ($save > 0) {
                    $colunas = array('id', 'SEQINCNFS');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['SEQINCNFS'],
                            'SEQWEB' => $return['id']
                        )
                    );
                    //gravarLog("Tabela: $table - " . $json . " - " . "SEQINCNFS: " . $dados['SEQINCNFS'] . " -  Empresa: " . $this->empresa_id . "; \n", $table);
            ...
            case 3: // Excluir registro
                ...
                //$condition = 'SEQINCNFS= ' . $post['SEQILX'] . ' AND id= ' . $post['SEQWEB'];

                $condition = 'SEQINCNFS =' . $post['SEQILX'];
                ...
                if ($error[2] != NULL) {
                    ...
                    //gravarLog("Tabela: $table - " . "SEQINCNFS: " . $post['SEQILX'] . "empresa = " . $this->empresa_id . "/n" . $json . "/n", $table);

                    return $json;
                } else {
                    if ($save >= 0) {
                        $json = json_encode(array('MSG' => 'Sucesso'));
                        //gravarLog("Tabela: $table - " . "SEQINCNFS: " . $post['SEQILX'] . " - " . "empresa = " . $this->empresa_id . " - " . $json, $table);
                        return $json;
                    }
                }
                break;
