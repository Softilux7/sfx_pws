    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            ...
            case 2:
                ...
                $condition = 'ID_TIPO_DEFEITO =' . $dados['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id;
                ...
                if ($save > 0) {
                    $colunas = array('id', 'ID_TIPO_DEFEITO');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['ID_TIPO_DEFEITO'],
                            'SEQWEB' => $return['id']
                        )
                    );
                    //gravarLog("Tabela: $table - " . $json . " - " . "ID_TIPO_DEFEITO: " . $dados['ID_TIPO_DEFEITO'] . " -  Empresa: " . $this->empresa_id . "; \n", $table);
            ...
            case 3: // Excluir registro
               ...
               // $condition = 'ID_TIPO_DEFEITO= ' . $post['SEQILX'] . ' AND id= ' . $post['SEQWEB'];

                $condition = 'ID_TIPO_DEFEITO =' . $post['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id;
                ...
                if ($error[2] != NULL) {
                    ...
                    //gravarLog("Tabela: $table - " . "ID_TIPO_DEFEITO: " . $post['SEQILX'] . "empresa = " . $this->empresa_id . "/n" . $json . "/n", $table);

                    return $json;
                } else {
                    if ($save >= 0) {
                        $json = json_encode(array('MSG' => 'Sucesso'));
                        //gravarLog("Tabela: $table - " . "ID_TIPO_DEFEITO: " . $post['SEQILX'] . " - " . "empresa = " . $this->empresa_id . " - " . $json, $table);
                        return $json;
                    }
                }
                break;
