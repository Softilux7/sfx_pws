    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            ...
            case 2:
                ...
                $condition = 'ID_ENTREGADOR =' . $dados['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id;
                ...
                if ($save > 0) {
                    $colunas = array('id', 'ID_ENTREGADOR');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['ID_ENTREGADOR'],
                            'SEQWEB' => $return['id']
                        )
                    );
                    gravarLog("Tabela: $table - " . $json . " - " . "ID_ENTREGADOR: " . $dados['ID_ENTREGADOR'] . " -  Empresa: " . $this->empresa_id . "; \n", $table);
            ...
            case 3: // Excluir registro
               ...
               // $condition = 'ID_ENTREGADOR= ' . $post['SEQILX'] . ' AND id= ' . $post['SEQWEB'];

                $condition = 'ID_ENTREGADOR =' . $post['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id;
                ...
                if ($error[2] != NULL) {
                    ...
                    gravarLog("Tabela: $table - " . "ID_ENTREGADOR: " . $post['SEQILX'] . "empresa = " . $this->empresa_id . "/n" . $json . "/n", $table);

                    return $json;
                } else {
                    if ($save >= 0) {
                        $json = json_encode(array('MSG' => 'Sucesso'));
                        gravarLog("Tabela: $table - " . "ID_ENTREGADOR: " . $post['SEQILX'] . " - " . "empresa = " . $this->empresa_id . " - " . $json, $table);
                        return $json;
                    }
                }
                break;
