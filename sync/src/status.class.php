    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            ...
            case 2:
                ...
                $condition = 'ID_STATUS_ORDEM_SERVICO =' . $dados['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id;
                ...
                if ($save > 0) {
                    $colunas = array('id', 'ID_STATUS_ORDEM_SERVICO');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['ID_STATUS_ORDEM_SERVICO'],
                            'SEQWEB' => $return['id']
                        )
                    );
                    //gravarLog("Tabela: $table - " . $json . " - " . "ID_STATUS_ORDEM_SERVICO: " . $dados['ID_STATUS_ORDEM_SERVICO'] . " -  Empresa: " . $this->empresa_id . "; \n", $table);
                    ...
                    $json = json_encode(array(
                        'MSG' => 'Erro',
                        'SEQILX' => $dados['ID_STATUS_ORDEM_SERVICO'],
                        'SEQWEB' => -1,
                        'SQL' => $this->db->sql,
                    ));
            ...
            case 3: // Excluir registro
                ...
                //$condition = 'ID_STATUS_ORDEM_SERVICO= ' . $post['SEQILX'] . ' AND id= ' . $post['SEQWEB'];

                $condition = 'ID_STATUS_ORDEM_SERVICO =' . $post['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id;
...
//                    gravarLog("Tabela: $table - " . "ID_STATUS_ORDEM_SERVICO: " . $post['SEQILX'] . "empresa = " . $this->empresa_id . "/n" . $json . "/n", $table);

                    return $json;
                } else {
                    if ($save >= 0) {
                        $json = json_encode(array('MSG' => 'Sucesso'));
  //                      gravarLog("Tabela: $table - " . "ID_STATUS_ORDEM_SERVICO: " . $post['SEQILX'] . " - " . "empresa = " . $this->empresa_id . " - " . $json, $table);
                        return $json;
                    }
                }
                break;
            ...
            case 4: // Retorno do sync ilux
                ...
                if (empty($post['ERR'])) {
                    $dados = array(
                        'ATUALIZADO' => '0',
                        'ERR' => '',
                        'ID_STATUS_ORDEM_SERVICO' => $post['SEQILX']
                    );
                } else {
