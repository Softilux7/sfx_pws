    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            ...
            case 2:
                ...
                $condition = 'ID_FICHA_TEC_ITEM =' . $dados['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id;
                ...
                if ($save > 0) {
                    $colunas = array('id', 'ID_FICHA_TEC_ITEM');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['ID_FICHA_TEC_ITEM'],
                            'SEQWEB' => $return['id']
                        )
                    );
                    //gravarLog("Tabela: $table - " . $json . " - " . "ID_FICHA_TEC_ITEM: " . $dados['ID_FICHA_TEC_ITEM'] . " -  Empresa: " . $this->empresa_id . "; \n", $table);
                    ...
                    $json = json_encode(array(
                        'MSG' => 'Erro',
                        'SEQILX' => $dados['ID_FICHA_TEC_ITEM'],
                        'SEQWEB' => -1,
                        'SQL' => $this->db->sql,
                    ));
            ...
            case 3: // Excluir registro
                ...
                //$condition = 'ID_FICHA_TEC_ITEM= ' . $post['SEQILX'] . ' AND id= ' . $post['SEQWEB'];

                $condition = 'ID_FICHA_TEC_ITEM =' . $post['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id;
                ...
                if ($error[2] != NULL) {
                    ...
                    //gravarLog("Tabela: $table - " . "ID_FICHA_TEC_ITEM: " . $post['SEQILX'] . "empresa = " . $this->empresa_id . "/n" . $json . "/n", $table);

                    return $json;
                } else {
                    if ($save >= 0) {
                        $json = json_encode(array('MSG' => 'Sucesso'));
                        //gravarLog("Tabela: $table - " . "ID_FICHA_TEC_ITEM: " . $post['SEQILX'] . " - " . "empresa = " . $this->empresa_id . " - " . $json, $table);
                        return $json;
                    }
                }
                break;
