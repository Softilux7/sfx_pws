...
class estoqueLocalTecnico extends RestController
{
    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            case 2:
                ...
                $condition = 'ID_LOC_ESTOQUE_X_TECNICO= ' . $dados['SEQILX'];
                ...
                if ($save > 0) {
                    $colunas = array('id', 'ID_LOC_ESTOQUE_X_TECNICO');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['ID_LOC_ESTOQUE_X_TECNICO'],
                            'SEQWEB' => $return['id']
                        )
                    );
            ...
            case 3: // Excluir registro
                ...
                $condition = 'ID_LOC_ESTOQUE_X_TECNICO= ' . $post['SEQILX'];
