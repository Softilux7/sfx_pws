        ...
        switch ($evt) {
            ...
            case 2:
                ...
                $dados['ATUALIZADO_ILUX'] = $dados['ATUALIZADO'];
                                $dados['empresa_id'] = $this->empresa_id;                 $dados['ID_BASE'] = $this->id_base;

                $condition = 'ID_CHECKLIST_PERGUNTA =' . $dados['SEQILX']. ' AND empresa_id= ' . $this->empresa_id;
                ...
                if ($save > 0) {
                    $colunas = array('id', 'ID_CHECKLIST_PERGUNTA');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['ID_CHECKLIST_PERGUNTA'],
                            'SEQWEB' => $return['id']
                        )
                    );
                    gravarLog("Tabela: $table - " . $json . " - " . "ID_CHECKLIST_PERGUNTA: " . $dados['ID_CHECKLIST_PERGUNTA'] . " -  Empresa: " . $this->empresa_id . "; \n",$table);
                    ...
                    $json = json_encode(array(
                        'MSG' => 'Erro',
                        'SEQILX' => $dados['ID_CHECKLIST_PERGUNTA'],
                        'SEQWEB' => -1,
                        'SQL' => $this->db->sql,
                    ));
            ...
            case 3: // Excluir registro
                ...
                $condition = 'ID_CHECKLIST_PERGUNTA= ' . $post['SEQILX'] . ' AND id= ' . $post['SEQWEB'] ;
                ...
                if ($error[2] != NULL) {
                    ...
                    gravarLog("Tabela: $table - " . "ID_CHECKLIST_PERGUNTA: " . $post['SEQILX'] . "empresa = ". $this->empresa_id ."/n" . $json . "/n",$table);

                    return $json;
                } else {
                    if ($save >= 0) {
                        $json = json_encode(array('MSG' => 'Sucesso'));
                        gravarLog("Tabela: $table - " . "ID_CHECKLIST_PERGUNTA: " . $post['SEQILX'] . " - " . "empresa = ". $this->empresa_id . " - " .  $json,$table);
                        return $json;
                    }
                }
                break;
            ...
            case 4: // Retorno do sync ilux
                ...
                if (empty($post['ERR'])) {
                    $dados = array(
                        'ATUALIZADO' => '0',
                        'ERR' => '',
                        'ID_CHECKLIST_PERGUNTA' => $post['SEQILX']
                    );
                } else {
