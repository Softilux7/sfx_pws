    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            ...
            case 2:
                ...
                $condition = 'ID_MEDIDOR =' . $dados['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id; // Atualizar com id do ilux
                ...
                if ($save > 0) {
                    $colunas = array('id', 'ID_MEDIDOR');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['ID_MEDIDOR'],
                            'SEQWEB' => $return['id']
                        )
                    );
                    //gravarLog("Tabela: $table - " . $json . " - " . "CDMEDIDOR: " . $dados['ID_MEDIDOR'] . " -  Empresa: " . $this->empresa_id . "; \n", $table);
            ...
            case 3: // Excluir registro
                ...
                //$condition = 'ID_MEDIDOR= ' . $post['SEQILX'] . ' AND id= ' . $post['SEQWEB'];

                $condition = 'ID_MEDIDOR =' . $post['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id; // Atualizar com id do ilux
                ...
                if ($error[2] != NULL) {
                    ...
                    //gravarLog("Tabela: $table - " . "ID_MEDIDOR: " . $post['SEQILX'] . "empresa = " . $this->empresa_id . "/n" . $json . "/n", $table);

                    return $json;
                } else {
                    if ($save >= 0) {
                        $json = json_encode(array('MSG' => 'Sucesso'));
                        //gravarLog("Tabela: $table - " . "ID_MEDIDOR: " . $post['SEQILX'] . " - " . "empresa = " . $this->empresa_id . " - " . $json, $table);
                        return $json;
                    }
                }
                break;
