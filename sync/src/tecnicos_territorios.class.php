 ...
 * Time: 16:38
 */
class tecnicosTerritorio extends RestController
{
    public $db = '';
    public $config = '';
    public $empresa_id = '';
    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            ...
            case 2:
                ...
                $condition = 'ID_SUPORTE_TERRITORIO =' . $dados['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id;
                ...
                if ($save > 0) {
                    $colunas = array('id', 'ID_SUPORTE_TERRITORIO');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['ID_SUPORTE_TERRITORIO'],
                            'SEQWEB' => $return['id']
                        )
                    );
                    //gravarLog("Tabela: $table - " . $json . " - " . "ID_SUPORTE_TERRITORIO: " . $dados['ID_SUPORTE_TERRITORIO'] . " -  Empresa: " . $this->empresa_id . "; \n", $table);
                    ...
                    $json = json_encode(array(
                        'MSG' => 'Erro',
                        'SEQILX' => $dados['ID_SUPORTE_TERRITORIO'],
                        'SEQWEB' => -1,
                        'SQL' => $this->db->sql,
                    ));
            ...
            case 3: // Excluir registro
                ...
                //$condition = 'ID_SUPORTE_TERRITORIO= ' . $post['SEQILX'] . ' AND id= ' . $post['SEQWEB'];

                $condition = 'ID_SUPORTE_TERRITORIO =' . $post['SEQILX'] . ' AND empresa_id= ' . $this->empresa_id;
                ...
                if ($error[2] != NULL) {
                    ...
                    //gravarLog("Tabela: $table - " . "ID_SUPORTE_TERRITORIO: " . $post['SEQILX'] . "empresa = " . $this->empresa_id . "/n" . $json . "/n", $table);

                    return $json;
                } else {
                    if ($save >= 0) {
                        $json = json_encode(array('MSG' => 'Sucesso'));
                       // gravarLog("Tabela: $table - " . "ID_SUPORTE_TERRITORIO: " . $post['SEQILX'] . " - " . "empresa = " . $this->empresa_id . " - " . $json, $table);
                        return $json;
                    }
                }
                break;
            ...
            case 4: // Retorno do sync ilux
                ...
                if (empty($post['ERR'])) {
                    $dados = array(
                        'ATUALIZADO' => '0',
                        'ERR' => '',
                        'ID_SUPORTE_TERRITORIO' => $post['SEQILX']
                    );
                } else {
