 ...
 * Time: 16:38
 */
class contratosIt extends RestController
{
    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            ...
            case 2:
                ...
                $condition = 'ID_CONTRATO_EQUIP = ' . $dados['SEQILX'];
                ...
                if ($save > 0) {
                    $colunas = array('id', 'ID_CONTRATO_EQUIP');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['ID_CONTRATO_EQUIP'],
                            'SEQWEB' => $return['id']
                        )
                    );
                    //gravarLog("Tabela: $table - " . $json . " - " . "ID_CONTRATO_EQUIP: " . $dados['ID_CONTRATO_EQUIP'] . " -  Empresa: " . $this->empresa_id . "; \n",$table);
                    ...
                    $json = json_encode(array(
                        'MSG' => 'Erro',
                        'SEQILX' => $dados['ID_CONTRATO_EQUIP'],
                        'SEQWEB' => -1,
                        'SQL' => $this->db->sql,
                    ));
            ...
            case 3: // Excluir registro
               ...
               // $condition = 'ID_CONTRATO_EQUIP= ' . $post['SEQILX'] . ' AND id= ' . $post['SEQWEB'] ;
               
               $condition = 'ID_CONTRATO_EQUIP = ' . $post['SEQILX'];
                ...
                if ($error[2] != NULL) {
                   ...
                   // gravarLog("Tabela: $table - " . "ID_CONTRATO_EQUIP: " . $post['SEQILX'] . "empresa = ". $this->empresa_id ."/n" . $json . "/n",$table);

                    return $json;
                } else {
                    if ($save >= 0) {
                        $json = json_encode(array('MSG' => 'Sucesso'));
                       // gravarLog("Tabela: $table - " . "ID_CONTRATO_EQUIP: " . $post['SEQILX'] . " - " . "empresa = ". $this->empresa_id . " - " .  $json,$table);
                        return $json;
                    }
                }
                break;
