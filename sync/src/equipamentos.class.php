 ...
 * Time: 16:38
 */
class equipamentos extends RestController
{
    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            ...
            case 2:
                ...
                $condition = 'CDEQUIPAMENTO =' . $dados['SEQILX'];
                ...
                if ($save > 0) {
                    ...
                    $colunas = array('id', 'CDEQUIPAMENTO');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['CDEQUIPAMENTO'],
                            'SEQWEB' => $return['id']
                        )
                    );
                    
                    gravarLog("Tabela: $table - " . $json . " - " . "CDEQUIPAMENTO: " . $dados['CDEQUIPAMENTO'] . " -  Empresa: " . $this->empresa_id . "; \n", $table);
            ...
            case 3: // Excluir registro
               ...
               // $condition = 'CDEQUIPAMENTO= ' . $post['SEQILX'] . ' AND id= ' . $post['SEQWEB'];

                $condition = 'CDEQUIPAMENTO =' . $post['SEQILX'];
                ...
                if ($error[2] != NULL) {
                    ...
                    gravarLog("Tabela: $table - " . "CDEQUIPAMENTO: " . $post['SEQILX'] . "empresa = " . $this->empresa_id . "/n" . $json . "/n", $table);

                    return $json;
                } else {
                    if ($save >= 0) {
                        $json = json_encode(array('MSG' => 'Sucesso'));
                        gravarLog("Tabela: $table - " . "CDEQUIPAMENTO: " . $post['SEQILX'] . " - " . "empresa = " . $this->empresa_id . " - " . $json, $table);
                        return $json;
                    }
                }
                break;
