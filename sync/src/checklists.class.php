 ...
 * Time: 16:38
 */
class checklists extends RestController
{
    ...
    public function index($index, $evt, $post)
    {
        ...
        switch ($evt) {
            ...
            case 2:
                ...
                $condition = 'ID_CHECKLIST =' . $dados['SEQILX'];
                ...
                if ($save > 0) {
                    $colunas = array('id', 'ID_CHECKLIST');
                    $return = $this->db->get_one($table, $condition, $colunas);

                    $json = json_encode(array(
                            'MSG' => 'Sucesso',
                            'SEQILX' => $return['ID_CHECKLIST'],
                            'SEQWEB' => $return['id']
                        )
                    );
                    gravarLog("Tabela: $table - " . $json . " - " . "ID_CHECKLIST: " . $dados['ID_CHECKLIST'] . " -  Empresa: " . $this->empresa_id . "; \n",$table);
            ...
            case 3: // Excluir registro
               ...
               // $condition = 'ID_CHECKLIST= ' . $post['SEQILX'] . ' AND id= ' . $post['SEQWEB'] ;

                $condition = 'ID_CHECKLIST =' . $post['SEQILX'];
                ...
                if ($error[2] != NULL) {
                    ...
                    gravarLog("Tabela: $table - " . "ID_CHECKLIST: " . $post['SEQILX'] . "empresa = ". $this->empresa_id ."/n" . $json . "/n",$table);

                    return $json;
                } else {
                    if ($save >= 0) {
                        $json = json_encode(array('MSG' => 'Sucesso'));
                        gravarLog("Tabela: $table - " . "ID_CHECKLIST: " . $post['SEQILX'] . " - " . "empresa = ". $this->empresa_id . " - " .  $json,$table);
                        return $json;
                    }
                }
                break;
